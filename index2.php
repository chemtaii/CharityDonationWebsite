...
if (!isset($_SESSION['ddname'])) {
    header("Location: index.html");
}else{
  $filter = $_SESSION['ddname'];
...
<head>
    <meta charset="utf-8">
    <title>Donation System - Delivery Driver Module</title>
    ...
    <div class="container-fluid py-5" id="data">
        <div class="container py-5">
            <div class="text-center mb-5">
                <h5 class="text-primary text-uppercase mb-3" style="letter-spacing: 5px;">View Database</h5>
...
$sql = "
    ...
    WHERE donations.Driver_ID = '$filter' OR donations.Status = 'Active'
";
...
?>
        <tr>
            ...
            <td><?php echo($row["Donor_Fullname"]); ?> contact on <a href="mailto:<?php echo($row["Donor_Email"]); ?>"><?php echo($row["Donor_Email"]); ?></a> or <?php echo($row["Donor_Phone"]); ?>.</td>
...
?>
<td><button id="delbtn" class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to accept to transport this donation?')?window.location.href='php/functions.php?action=acceptD&id=<?php echo($row["Donation_ID"]); ?>':true;" title='Accept Donation'>Accept</button></td>
<td><button id="delbtn1" class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to deny to transport this donation?')?window.location.href='php/functions.php?action=denyD&id=<?php echo($row["Donation_ID"]); ?>':true;" title='Deny Donation'>Deny</button></td>
<?php
